...
import { checkOrgName, initiateSignupOTP, validateSignupOTP, completeSubscription, checkEmail, checkUsername } from '@/app/serverActions/StarterSubscribeServerActions/SubscribeSignup'
import '../../Subscriber/SubscribeSignup/SubscribeSignup.css' 
...
const CompanyForm = () => {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
        ...
        const result = await completeSubscription(finalPayload);
        if (result.success) {
      ...
      setError('Critical error during creation.');
    } finally {
      setLoading(false);
    }
  };
        ...
        {step === 1 && (
          <form className="subscribersignupForm" onSubmit={handleStep1Submit}>
            ...
            <div className="subscribersignupInputGroup">
              ...
              {/* UPDATED INPUT: accept only jpg/jpeg */}
              <input 
                type="file" 
                name="logo" 
        ...
        {step === 4 && (
          <form className="subscribersignupForm" onSubmit={handleFinalSubmit}>
            ...
            <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
              ...
              <button type="submit" className="subscribersignupButton" disabled={loading}>{loading ? 'Creating Account...' : 'Complete Signup'}</button>
            </div>
          </form>
        )}
