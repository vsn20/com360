...
const FDNS_Document = ({ id, documents: initialDocuments, onDocumentsUpdate, document_types, document_purposes, document_subtypes }) => {
  ...
  const handleDeleteDocument = async (docId) => {
    if (!window.confirm("Are you sure you want to delete this FDNS document? This action cannot be undone.")) {
      return;
    }
    try {
      await deleteFdnsDocument(docId); // Use deleteFdnsDocument
      setEditing(false);
      setEditDocument(null);
      setOriginalDocument(null);
      setIsViewOnly(false);
      ...
      setError(`Failed to delete FDNS document: ${err.message}`);
    }
  };
    ...
    <div>
      <div className={styles.titleContainer}>
        <h3 className={styles.title}>
          {editing ? (isViewOnly ? 'FDNS Document Details' : (editDocument ? 'Edit FDNS Document' : 'Add New FDNS Document')) : 'FDNS Documents'}
        </h3>
        {!editing && (
      ...
      {editing ? (
        ...
        <>
          ...
          {filteredDocuments.length === 0 ? (
            <p className={styles.emptyState}>No FDNS documents available.</p>
          ) : (
            <>
