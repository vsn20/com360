// app/serverActions/Employee/EmployeePafDocument.js
...
export async function addPafDocument(formData) {
  try {
    ...
    // Store PAF documents in a separate folder
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'paf_documents');
    const filePath = path.join(uploadDir, filename);

    await fs.mkdir(uploadDir, { recursive: true });
    await fs.writeFile(filePath, Buffer.from(await file.arrayBuffer()));

    const documentPath = `/uploads/paf_documents/${filename}`;

    const pool = await DBconnection();
    const [result] = await pool.query(
      `INSERT INTO C_EMP_PAF (empid, orgid, document_name, document_type, subtype, document_path, document_purpose, comments, startdate, enddate, created_by, updated_by, created_date, last_updated_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [empId, orgId, documentName, documentType, subtype, documentPath, documentPurpose, comments, startdate, enddate, userId, userId]
    );
    return result.insertId;
  } catch (error) {
    console.error('Error adding PAF document:', error.message);
    throw new Error(`Failed to add PAF document: ${error.message}`);
  }
}

export async function updatePafDocument(formData) {
  try {
    const cookieStore = cookies();
    const token = cookieStore.get('jwt_token')?.value;
    ...
    const pool = await DBconnection();
    let updateQuery = `UPDATE C_EMP_PAF SET updated_by = ?, last_updated_date = NOW()`;
    const params = [userId];
    ...
    if (result.affectedRows === 0) {
      throw new Error('PAF Document not found or unauthorized.');
    }
    return true;
  } catch (error) {
    console.error('Error updating PAF document:', error.message);
    throw new Error(`Failed to update PAF document: ${error.message}`);
  }
}

export async function deletePafDocument(id) {
  try {
    const cookieStore = cookies();
    const token = cookieStore.get('jwt_token')?.value;
    ...
    const [docResult] = await pool.query(
      `SELECT document_path FROM C_EMP_PAF WHERE id = ? AND orgid = ?`,
      [id, orgId]
    );
    ...
    const [result] = await pool.query(
      `DELETE FROM C_EMP_PAF WHERE id = ? AND orgid = ?`,
      [id, orgId]
    );

    if (result.affectedRows === 0) {
      throw new Error('PAF Document not found or unauthorized to delete.');
    }
    return true;
  } catch (error) {
    console.error('Error deleting PAF document:', error.message);
    throw new Error(`Failed to delete PAF document: ${error.message}`);
  }
}