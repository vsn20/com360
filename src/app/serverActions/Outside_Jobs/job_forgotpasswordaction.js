...
export async function forgotpasswordaction(formData) {
  ...
  // Ensure OTP table exists in Central DB
  try {
  ...
  // ---------------------------------------------------------
  // STEP 1: IDENTIFY USER & SEND OTP
  // ---------------------------------------------------------
  if (step === 'identifier') {
    const email = formData.get('email');

    // Check if email exists in C_CANDIDATE (Central DB)
    const [countCandidate] = await metaPool.query(
      `SELECT COUNT(*) AS count FROM C_CANDIDATE WHERE email=?`, 
      [email]
    );
  ...
  if (step === 'otp') {
    ...
    const [rows] = await metaPool.query(
      `SELECT otp, expiry FROM C_FORGOTPASSWORD_JOBS_OUTSIDE WHERE email=?`, 
      [email]
    );
    ...
    const { otp: storedOTP, expiry } = rows[0];
    if (new Date() > new Date(expiry)) {
      await metaPool.query(`DELETE FROM C_FORGOTPASSWORD_JOBS_OUTSIDE WHERE email=?`, [email]);
      return { success: false, error: "OTP has expired." };
    }
  ...
  if (step === 'reset') {
    ...
    // Update password in C_CANDIDATE (Central DB)
    try {
      await metaPool.query(
        `UPDATE C_CANDIDATE SET password=? WHERE email=?`, 
